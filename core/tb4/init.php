...
require_once __DIR__ . '/modules/media/videoslideritemmodule.php';
...
require_once __DIR__ . '/modules/content/socialitemmodule.php';
...
require_once __DIR__ . '/modules/content/signupitemmodule.php';
...
require_once __DIR__ . '/modules/content/barcounteritemmodule.php';
require_once __DIR__ . '/modules/content/filterportfoliomodule.php';
require_once __DIR__ . '/modules/content/shopmodule.php';
require_once __DIR__ . '/modules/content/mapmodule.php';
require_once __DIR__ . '/modules/content/mapitemmodule.php';
require_once __DIR__ . '/modules/content/menumodule.php';
require_once __DIR__ . '/modules/content/sidebarmodule.php';
...
require_once __DIR__ . '/modules/interactive/contactitemmodule.php';
...
class ModuleRegistry {
    ...
    public function getModulesForJson(): array {
        ...
        foreach ($this->modules as $slug => $module) {
            // Get typography fields and convert to frontend format
            $typography_fields = [];
            $raw_typography = $module->get_typography_fields();
            if (!empty($raw_typography)) {
                foreach ($raw_typography as $key => $config) {
                    $typography_fields[] = [
                        'key' => $key,
                        'label' => $config['label'] ?? ucfirst(str_replace('_', ' ', $key)) . ' Typography',
            ...
            // Get custom CSS fields and convert to frontend format
            $custom_css_fields = [];
            $raw_css = $module->get_custom_css_fields();
            if (!empty($raw_css)) {
                foreach ($raw_css as $key => $config) {
                    $custom_css_fields[] = [
                        'key' => $key,
            ...
            $result[$slug] = [
                ...
                'typography_fields' => $typography_fields,
                'custom_css_fields' => $custom_css_fields
            ];
        }
        return $result;
    }
...
$registry->register(\Core\TB4\Modules\Media\VideoSliderItemModule::class);
...
$registry->register(\Core\TB4\Modules\Content\SocialItemModule::class);
...
$registry->register(\Core\TB4\Modules\Content\SignupItemModule::class);
...
$registry->register(\Core\TB4\Modules\Content\BarCounterItemModule::class);
...
$registry->register(\Core\TB4\Modules\Content\MapModule::class);
$registry->register(\Core\TB4\Modules\Content\MapItemModule::class);
$registry->register(\Core\TB4\Modules\Content\MenuModule::class);
$registry->register(\Core\TB4\Modules\Content\SidebarModule::class);
...
$registry->register(\Core\TB4\Modules\Interactive\ContactItemModule::class);
