...
if (!function_exists('theme_studio_preview_script')) {
    /**
     * Generate JS for Theme Studio live preview.
     * Injected into layout when THEME_STUDIO_PREVIEW is defined.
     * Listens for postMessage from parent (Theme Studio sidebar)
     * and applies CSS variable overrides + field updates in real-time.
     */
    ...
    function applyValues(vals) {
        ...
        if (vals.brand) {
            ...
            if (vals.brand.site_name !== undefined) {
                ...
                /* Update hero title if it matches site name */
        ...
        if (vals.header) {
            if (vals.header.cta_text !== undefined) {
                setText(".header-cta, .cta-button, nav .btn-primary, header .btn, a.cta, .nav-cta", vals.header.cta_text);
            }
            if (vals.header.cta_url !== undefined) {
                document.querySelectorAll(".header-cta, .cta-button, nav .btn-primary, header .btn, a.cta, .nav-cta").forEach(function(el) {
                    if (el.tagName === "A") el.href = vals.header.cta_url;
                });
            }
        }
        ...
        if (vals.hero) {
            ...
            if (vals.hero.subheadline !== undefined) {
                setText(".hero p, .hero-subtitle, .hero-description, section.hero p, .hero-content p", vals.hero.subheadline);
            }
            if (vals.hero.background_image !== undefined) {
                document.querySelectorAll(".hero, section.hero, .hero-section").forEach(function(el) {
                    if (vals.hero.background_image) el.style.backgroundImage = "url(" + vals.hero.background_image + ")";
                });
            }
        }
