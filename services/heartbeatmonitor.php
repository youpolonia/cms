...
class HeartbeatMonitor {
    ...
    public function __construct($db, NotificationService $notificationService, array $config = []) {
        $this->db = $db;
    ...
    private function getFailureThreshold() {
        return $this->config['default_threshold'] ?? 300;
    }
    ...
    private function determineAlertLevel($failureCount) {
        $maxLevel = $this->config['max_alert_level'] ?? 5;
        return min(ceil($failureCount / 3), $maxLevel);
    }
