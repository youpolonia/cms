...
class HuggingFaceProvider implements AIProviderInterface {
    ...
    public function generateContent(
        ...
        if (is_array($response)) {
            ...
            $this->trackUsage($tenantId, $model, strlen($prompt), strlen($generatedText));
    ...
    public function fineTuneModel(
        ...
        $response = $this->makeRequest('fine-tune', [
            'base_model' => $baseModel,
            'training_data' => $this->prepareTrainingData($trainingData),
    ...
    private function makeRequest(string $endpoint, array $data): array {
        $ch = curl_init("https://api-inference.huggingface.co/models/{$endpoint}");
        ...
        if (curl_errno($ch)) {
            throw new RuntimeException('HuggingFace API error: ' . curl_error($ch));
        }

        $httpCode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
        if ($httpCode >= 400) {
            throw new RuntimeException("HuggingFace API returned HTTP {$httpCode}");
        }
