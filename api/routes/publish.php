...
try {
    ...
    // Lock content during publish
    $locked = TenantValidator::lockContent($contentId, $tenantId);
    if (!$locked) {
        throw new Exception('Failed to lock content for publishing');
    }

    ContentPublisher::init();
    $result = ContentPublisher::publish((int)$contentId);
    ...
    error_log('publish route error: ' . $e->getMessage());
    echo json_encode([
        'status' => 'error',
