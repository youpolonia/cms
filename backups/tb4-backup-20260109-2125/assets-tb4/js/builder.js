...
const TB4Builder = {
    ...
    initCanvas() {
        ...
        // Filterable Portfolio click handler (event delegation)
        this.dom.canvas.addEventListener('click', (e) => {
            const filterBtn = e.target.closest('.tb4-filter-btn');
            
            if (filterBtn) {
                e.stopPropagation();
                const filterPortfolio = filterBtn.closest('.tb4-filter-portfolio-preview');
                
                if (filterPortfolio) {
                    const allBtns = filterPortfolio.querySelectorAll('.tb4-filter-btn');
                    const allItems = filterPortfolio.querySelectorAll('.tb4-filter-item');
                    const filterValue = filterBtn.dataset.filter;
                    
                    allBtns.forEach(btn => {
                        btn.classList.remove('active');
                        if (btn.style.borderBottomColor) {
                    ...
                    if (filterBtn.style.borderBottomColor !== undefined && filterBtn.style.background === 'transparent') {
                        filterBtn.style.borderBottomColor = '#2563eb';
        ...
        // Filterable Portfolio hover effect
        this.dom.canvas.addEventListener('mouseover', (e) => {
            const filterItem = e.target.closest('.tb4-filter-item');
            if (filterItem) {
                const overlay = filterItem.querySelector('.tb4-filter-item-overlay');
    ...
    renderModuleContent(module) {
        ...
        switch (moduleType) {
            case 'text':
                if (data.text && data.text.trim() && data.text !== '<p>Click to edit text...</p>') {
                    return `<div class="tb4-text-content">${data.text}</div>`;
                }
            ...
            case 'map':
                return `<div class="tb4-map-placeholder" style="aspect-ratio:16/9;background:linear-gradient(135deg,#d1fae5,#a7f3d0);border-radius:12px;position:relative;overflow:hidden;">
                    <div style="position:absolute;inset:0;opacity:0.3;">
                        <svg width="100%" height="100%" viewBox="0 0 400 225">
                            <path d="M0,100 Q100,50 200,100 T400,100" stroke="#059669" fill="none" stroke-width="2"/>
                            <path d="M0,150 Q100,100 200,150 T400,150" stroke="#059669" fill="none" stroke-width="1.5"/>
                            <circle cx="150" cy="90" r="8" fill="#059669"/>
                            <circle cx="280" cy="120" r="6" fill="#10b981"/>
                            <rect x="50" y="160" width="40" height="30" fill="#d1fae5" stroke="#059669"/>
                            <rect x="300" y="70" width="50" height="40" fill="#d1fae5" stroke="#059669"/>
                        </svg>
                    </div>
                    <div style="position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;">
                        <div style="width:48px;height:48px;background:#059669;border-radius:50% 50% 50% 0;transform:rotate(-45deg);display:flex;align-items:center;justify-content:center;margin-bottom:8px;">
                            <div style="width:16px;height:16px;background:white;border-radius:50%;transform:rotate(45deg);"></div>
                        </div>
                        <span style="color:#065f46;font-weight:600;font-size:16px;">Map Location</span>
                        <span style="color:#047857;font-size:13px;margin-top:4px;">${this.escapeHtml(data.address || 'Set address in settings')}</span>
                    </div>
                </div>`;
            ...
            case 'shop': {
                ...
                for (let pi = 1; pi <= 4; pi++) {
                    let pTitle = data['product' + pi + '_title'] || '';
                    let pImage = data['product' + pi + '_image'] || '';
                    let pPrice = data['product' + pi + '_price'] || '';
                    let pSalePrice = data['product' + pi + '_sale_price'] || '';
                    let pLink = data['product' + pi + '_link'] || '#';
                    if (pTitle.trim()) {
                        shopProducts.push({
                            title: pTitle,
                            image: pImage,
                ...
                if (shopProducts.length === 0) {
                    shopProducts = [
                        {title: 'Classic T-Shirt', image: '', price: '$29.99', salePrice: '', link: '#'},
                        {title: 'Leather Jacket', image: '', price: '$199.99', salePrice: '$149.99', link: '#'},
                        {title: 'Running Shoes', image: '', price: '$89.99', salePrice: '', link: '#'},
                        {title: 'Denim Jeans', image: '', price: '$59.99', salePrice: '$39.99', link: '#'}
                    ];
                }

                let shopColumns = data.columns || '4';
                let shopGap = data.gap || '24px';
                let shopCardBg = data.card_bg_color || '#ffffff';
                let shopCardRadius = data.card_border_radius || '12px';
                let shopCardShadow = data.card_shadow || 'sm';
                let shopCardBorder = data.card_border || 'light';
                let shopImageHeight = data.image_height || '200px';
                let shopImageFit = data.image_fit || 'cover';
                let shopTitleColor = data.title_color || '#111827';
                let shopTitleSize = data.title_font_size || '16px';
                let shopPriceColor = data.price_color || '#111827';
                let shopPriceSize = data.price_font_size || '18px';
                let shopSalePriceColor = data.sale_price_color || '#dc2626';
                let shopOriginalColor = data.original_price_color || '#9ca3af';
                let shopBadgeBg = data.sale_badge_bg || '#dc2626';
                let shopBadgeColor = data.sale_badge_color || '#ffffff';
                let shopBtnBg = data.button_bg_color || '#2563eb';
                let shopBtnColor = data.button_text_color || '#ffffff';
                let shopBtnRadius = data.button_border_radius || '8px';
                let shopBtnFull = data.button_full_width !== 'no';
                let shopHoverEffect = data.hover_effect || 'lift';
                let shopShowBadge = data.show_sale_badge !== 'no';
                let shopShowCart = data.show_add_to_cart !== 'no';
                let shopCartText = data.add_to_cart_text || 'Add to Cart';
                ...
                let shopShadow = shadowMap[shopCardShadow] || shadowMap['sm'];
                ...
                let shopBorderVal = borderMap[shopCardBorder] || borderMap['light'];
                ...
                let shopHtml = '<div class="tb4-shop-preview" style="display:grid;grid-template-columns:repeat(' + shopColumns + ',1fr);gap:' + shopGap + ';">';

                shopProducts.forEach(function(product, index) {
                    let hasSale = product.salePrice && product.salePrice.trim() !== '';
                    let hoverClass = shopHoverEffect !== 'none' ? ' hover-' + shopHoverEffect : '';
                    let imageBg = product.image ? 'url(' + product.image + ')' : gradients[index % gradients.length];

                    shopHtml += '<div class="tb4-shop-card' + hoverClass + '" style="background:' + shopCardBg + ';border-radius:' + shopCardRadius + ';overflow:hidden;box-shadow:' + shopShadow + ';border:' + shopBorderVal + ';transition:transform 0.3s ease,box-shadow 0.3s ease;">';
                    
                    // Image
                    shopHtml += '<div class="tb4-shop-card-image" style="position:relative;width:100%;height:' + shopImageHeight + ';overflow:hidden;background:#f3f4f6;">';
                    shopHtml += '<div class="tb4-shop-card-image-placeholder" style="width:100%;height:100%;background:' + imageBg + ';background-size:' + shopImageFit + ';background-position:center;transition:transform 0.3s ease;"></div>';
                    
                    if (shopShowBadge && hasSale) {
                        shopHtml += '<span class="tb4-shop-sale-badge" style="position:absolute;top:12px;left:12px;padding:4px 12px;background:' + shopBadgeBg + ';color:' + shopBadgeColor + ';font-size:12px;font-weight:600;border-radius:4px;text-transform:uppercase;">Sale</span>';
                    }
                    shopHtml += '</div>';

                    // Content
                    shopHtml += '<div class="tb4-shop-card-content" style="padding:16px;">';
                    shopHtml += '<h4 class="tb4-shop-card-title" style="font-size:' + shopTitleSize + ';font-weight:600;color:' + shopTitleColor + ';margin:0 0 8px 0;">' + product.title + '</h4>';
                    
                    // Price
                    shopHtml += '<div class="tb4-shop-card-price" style="display:flex;align-items:center;gap:8px;margin-bottom:12px;">';
                    if (hasSale) {
                        shopHtml += '<span class="tb4-shop-price-sale" style="font-size:' + shopPriceSize + ';font-weight:700;color:' + shopSalePriceColor + ';">' + product.salePrice + '</span>';
                        shopHtml += '<span class="tb4-shop-price-original" style="font-size:14px;color:' + shopOriginalColor + ';text-decoration:line-through;">' + product.price + '</span>';
                    } else {
                        shopHtml += '<span class="tb4-shop-price-current" style="font-size:' + shopPriceSize + ';font-weight:700;color:' + shopPriceColor + ';">' + product.price + '</span>';
                    }
                    shopHtml += '</div>';

                    // Add to Cart button
                    if (shopShowCart) {
                        let btnWidth = shopBtnFull ? 'width:100%;' : '';
                        shopHtml += '<button class="tb4-shop-add-btn" style="display:inline-flex;align-items:center;justify-content:center;gap:8px;padding:10px 20px;background:' + shopBtnBg + ';color:' + shopBtnColor + ';border:none;border-radius:' + shopBtnRadius + ';font-size:14px;font-weight:600;cursor:pointer;' + btnWidth + '">';
                        shopHtml += '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="9" cy="21" r="1"/><circle cx="20" cy="21" r="1"/><path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6"/></svg>';
                        shopHtml += shopCartText + '</button>';
                    }
                ...
                shopHtml += '</div>';
                shopHtml += '<style>.tb4-shop-card.hover-lift:hover{transform:translateY(-8px);box-shadow:0 20px 40px rgba(0,0,0,0.15)}.tb4-shop-card.hover-zoom:hover .tb4-shop-card-image-placeholder{transform:scale(1.1)}</style>';
            ...
            case 'filter_portfolio': {
                let filterPortItems = [];
                let filterPortCategories = new Set();
                
                for (let fi = 1; fi <= 6; fi++) {
                    let fTitle = data['item' + fi + '_title'] || '';
                    let fCategory = data['item' + fi + '_category'] || '';
                    let fImage = data['item' + fi + '_image'] || '';
                    let fLink = data['item' + fi + '_link'] || '#';
                    if (fTitle.trim()) {
                        filterPortItems.push({
                            title: fTitle,
                            category: fCategory,
                            image: fImage,
                            link: fLink
                        ...
                        if (fCategory.trim()) {
                            filterPortCategories.add(fCategory);
                        }
                    }
                }

                if (filterPortItems.length === 0) {
                    filterPortItems = [
                    ...
                    filterPortCategories = new Set(['Branding', 'Web Design', 'Development']);
                }

                let filterPortColumns = data.columns || '3';
                let filterPortGap = data.gap || '24px';
                let filterPortAlignment = data.filter_alignment || 'center';
                let filterPortStyle = data.filter_style || 'pills';
                let filterPortFilterBg = data.filter_bg_color || '#f3f4f6';
                let filterPortActiveBg = data.filter_active_bg || '#2563eb';
                let filterPortTextColor = data.filter_text_color || '#374151';
                let filterPortActiveText = data.filter_active_text || '#ffffff';
                let filterPortFilterGap = data.filter_gap || '8px';
                let filterPortFilterMargin = data.filter_margin_bottom || '32px';
                let filterPortItemBg = data.item_bg_color || '#1f2937';
                let filterPortRadius = data.item_border_radius || '12px';
                let filterPortOverlay = data.overlay_color || 'rgba(37, 99, 235, 0.9)';
                let filterPortTitleColor = data.title_color || '#ffffff';
                let filterPortTitleSize = data.title_font_size || '18px';
                let filterPortCatColor = data.category_color || 'rgba(255,255,255,0.8)';
                let filterPortShowAll = data.show_all_filter !== 'no';
                let filterPortAllText = data.all_filter_text || 'All';

                let justifyMap = {left: 'flex-start', center: 'center', right: 'flex-end'};
                let filterPortJustify = justifyMap[filterPortAlignment] || 'center';

                let filterPortHtml = '<div class="tb4-filter-portfolio-preview">';
                
                filterPortHtml += '<div class="tb4-filter-buttons" style="display:flex;flex-wrap:wrap;gap:' + filterPortFilterGap + ';justify-content:' + filterPortJustify + ';margin-bottom:' + filterPortFilterMargin + ';">';
                
                let btnBaseStyle = '';
                let btnActiveStyle = '';
                let btnRadius = filterPortStyle === 'pills' ? '20px' : (filterPortStyle === 'buttons' ? '8px' : '0');
                
                if (filterPortStyle === 'underline') {
                    btnBaseStyle = 'background:transparent;border:none;border-bottom:2px solid transparent;border-radius:0;padding:8px 16px;';
                    btnActiveStyle = 'background:transparent;color:' + filterPortActiveBg + ';border-bottom-color:' + filterPortActiveBg + ';';
                } else {
                    btnBaseStyle = 'background:' + filterPortFilterBg + ';border:none;border-radius:' + btnRadius + ';padding:8px 20px;';
                    btnActiveStyle = 'background:' + filterPortActiveBg + ';color:' + filterPortActiveText + ';';
                }

                if (filterPortShowAll) {
                    filterPortHtml += '<button class="tb4-filter-btn active" data-filter="all" style="' + btnBaseStyle + btnActiveStyle + 'font-size:14px;font-weight:500;cursor:pointer;transition:all 0.3s;">' + filterPortAllText + '</button>';
                }
                
                Array.from(filterPortCategories).forEach(function(cat, idx) {
                    let isFirst = !filterPortShowAll && idx === 0;
                    let style = isFirst ? btnBaseStyle + btnActiveStyle : btnBaseStyle + 'color:' + filterPortTextColor + ';';
                    let activeClass = isFirst ? ' active' : '';
                    filterPortHtml += '<button class="tb4-filter-btn' + activeClass + '" data-filter="' + cat.toLowerCase().replace(/\s+/g, '-') + '" style="' + style + 'font-size:14px;font-weight:500;cursor:pointer;transition:all 0.3s;">' + cat + '</button>';
                });
                
                filterPortHtml += '</div>';

                filterPortHtml += '<div class="tb4-filter-grid" style="display:grid;grid-template-columns:repeat(' + filterPortColumns + ',1fr);gap:' + filterPortGap + ';">';
                ...
                filterPortItems.forEach(function(item, index) {
                    let itemBg = item.image ? 'url(' + item.image + ')' : gradients[index % gradients.length];
                    let catSlug = item.category.toLowerCase().replace(/\s+/g, '-');
                    
                    filterPortHtml += '<div class="tb4-filter-item" data-category="' + catSlug + '" style="position:relative;border-radius:' + filterPortRadius + ';overflow:hidden;aspect-ratio:4/3;background:' + filterPortItemBg + ';">';
                    filterPortHtml += '<div class="tb4-filter-item-inner" style="width:100%;height:100%;background:' + itemBg + ';background-size:cover;background-position:center;"></div>';
                    filterPortHtml += '<div class="tb4-filter-item-overlay" style="position:absolute;inset:0;background:' + filterPortOverlay + ';display:flex;flex-direction:column;align-items:center;justify-content:center;opacity:0;transition:opacity 0.3s;">';
                    filterPortHtml += '<h4 style="font-size:' + filterPortTitleSize + ';font-weight:600;color:' + filterPortTitleColor + ';margin:0 0 4px 0;text-align:center;">' + item.title + '</h4>';
                    filterPortHtml += '<span style="font-size:14px;color:' + filterPortCatColor + ';">' + item.category + '</span>';
                    filterPortHtml += '</div></div>';
                });

                filterPortHtml += '</div></div>';
                return filterPortHtml;
            }
        ...
        switch (field.type) {
            ...
            case 'select':
                ...
                if (Array.isArray(rawOptions)) {
                    ...
                    // Object format: {value: 'label', ...} from PHP
