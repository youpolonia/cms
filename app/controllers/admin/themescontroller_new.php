...
class ThemesController
{
    ...
    public function delete(Request $request): void
    {
        ...
        if (in_array($slug, $protected)) {
            Session::flash('error', "Theme '{$slug}' is protected.");
            Response::redirect('/admin/themes');
            return;
        }
        $activeTheme = $this->getActiveTheme();
        ...
        if ($this->deleteDirectory($realThemePath)) {
            Session::flash('success', "Theme '{$slug}' deleted!");
        } else {
    ...
    public function customize(Request $request): void
    {
        ...
        $hasCustomize = !empty($supports['blank-canvas']) || !empty($supports['custom-header']) || 
                        !empty($supports['custom-footer']) || !empty($defaultOptions);
