...
if (!$menu) {
    $_SESSION['flash_message'] = 'Menu not found';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            $stmt = $pdo->prepare("UPDATE menus SET name = :name, slug = :slug WHERE id = :id");
            ...
            $success = true;
            $_SESSION['flash_message'] = 'Menu updated successfully';
            header('Location: menus.php');
            exit;
...
<body>
 ...
 required 
                       value="<?= isset($_POST['name']) ? htmlspecialchars($_POST['name']) : htmlspecialchars($menu['name']) ?>">
?>            </div>
 ...
 required 
                       value="<?= isset($_POST['slug']) ? htmlspecialchars($_POST['slug']) : htmlspecialchars($menu['slug']) ?>">
