...
<div class="container mx-auto px-4 py-8">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold">Edit Approval Step</h1>
    ...
    <div class="bg-white shadow overflow-hidden sm:rounded-lg">
        <div class="px-4 py-5 sm:p-6">
            <form action="{{ route('theme-approval-steps.update', $step) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <input type="number" name="timeout_days" id="timeout_days" min="1" value="{{ old('timeout_days', $step->timeout_days) }}"
                    ...
                    <div class="pt-2">
                        ...
                        <textarea name="description" id="description" rows="3"
                                  class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">{{ old('description', $step->description) }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="approver_role" id="approver_role" required
                            ...
                            @foreach($roles as $role)
                                <option value="{{ $role->name }}" @selected(old('approver_role', $step->approver_role) == $role->name)>
                                    {{ $role->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="number" name="required_approvals" id="required_approvals" min="1" 
                               value="{{ old('required_approvals', $step->required_approvals) }}" required
                               class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                    </div>
                    ...
                    <div class="flex items-center">
                        <input type="checkbox" name="is_required" id="is_required" value="1"
                               @checked(old('is_required', $step->is_required))
                    ...
                    <div>
                        ...
                        <select name="approval_logic" id="approval_logic" required
                                class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                            <option value="any" @selected(old('approval_logic', $step->approval_logic) == 'any')>Any approval satisfies step</option>
                            <option value="all" @selected(old('approval_logic', $step->approval_logic) == 'all')>All approvals required</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="rejection_logic" id="rejection_logic" required
                                class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                            <option value="any" @selected(old('rejection_logic', $step->rejection_logic) == 'any')>Any rejection fails step</option>
                            <option value="all" @selected(old('rejection_logic', $step->rejection_logic) == 'all')>All rejections required</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="timeout_days" id="timeout_days" min="1"
                               value="{{ old('timeout_days', $step->timeout_days) }}"
                               class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                    </div>
                    ...
                    <div>
                        ...
                        <div id="requirements-container" class="mt-2 space-y-2">
                            @foreach(old('requirements', $step->requirements ?? []) as $requirement)
                            <div class="flex">
                                <input type="text" name="requirements[]" value="{{ $requirement }}" placeholder="Enter a requirement"
