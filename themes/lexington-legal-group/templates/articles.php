...
<div class="atpl-wrap">
    ...
    <aside class="llg-articles-sidebar">
    <div class="llg-sidebar-search">
        <form class="llg-search-form" action="/articles" method="GET">
            <input type="text" name="q" placeholder="Search articles..." class="llg-search-input">
            <button type="submit" class="llg-search-btn" aria-label="Search">
                <i class="fas fa-search"></i>
            </button>
        </form>
    </div>

    <div class="llg-sidebar-block">
        <h4 class="llg-sidebar-title">Categories</h4>
        <div class="llg-categories-list">
            <?php if (!empty($categories)): ?>
                <?php foreach ($categories as $cat): ?>
                    <a href="/articles?category=<?= esc($cat['slug']) ?>" class="llg-category-link">
                        <span class="llg-category-name"><?= esc($cat['name']) ?></span>
                        <span class="llg-category-count"><?= (int)($cat['article_count'] ?? 0) ?></span>
                    </a>
                <?php endforeach; ?>
            <?php else: ?>
                <p class="llg-sidebar-empty">No categories yet.</p>
            <?php endif; ?>
        </div>
    </div>

    <div class="llg-sidebar-block">
        <h4 class="llg-sidebar-title">About Our Insights</h4>
        <p class="llg-sidebar-about" data-ts="sidebar.about">
            <?= esc(theme_get('sidebar.about', 'Expert analysis and commentary on corporate law, intellectual property, and regulatory developments from our team of seasoned attorneys.')) ?>
        </p>
    </div>

    <div class="llg-sidebar-block">
        <h4 class="llg-sidebar-title">Stay Informed</h4>
        <p class="llg-sidebar-newsletter-desc">Receive our latest legal insights directly to your inbox.</p>
        <form class="llg-newsletter-form">
            <input type="email" placeholder="Your professional email" class="llg-newsletter-input" required>
            <button type="button" class="llg-btn llg-btn--small llg-btn--primary">Subscribe</button>
        </form>
    </div>

    <div class="llg-sidebar-block llg-sidebar-quote">
        <i class="fas fa-quote-left llg-quote-icon"></i>
        <blockquote class="llg-quote-text" data-ts="sidebar.quote">
            <?= esc(theme_get('sidebar.quote', 'In law, what is done skillfully seems effortless.')) ?>
        </blockquote>
        <cite class="llg-quote-cite">— Legal Maxim</cite>
    </div>
</aside>
</div>