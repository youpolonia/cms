...
<div class="atpl-wrap">
    ...
    <aside class="lsa-sidebar">
    <div class="lsa-sidebar-widget lsa-sidebar-search">
        <h4 class="lsa-sidebar-title">Search Articles</h4>
        <form class="lsa-search-form" action="/articles" method="get">
            <div class="lsa-search-input-wrap">
                <input type="text" name="q" placeholder="Type keywords..." class="lsa-search-input">
                <button type="submit" class="lsa-search-btn" aria-label="Search">
    ...
    <div class="lsa-sidebar-widget lsa-sidebar-categories">
        <h4 class="lsa-sidebar-title">Language Topics</h4>
        <div class="lsa-categories-list">
            <?php if (!empty($categories)): ?>
                <?php foreach ($categories as $cat): ?>
                    <a href="/articles?category=<?= esc($cat['slug']) ?>" class="lsa-category-link">
                        <span class="lsa-category-name"><?= esc($cat['name']) ?></span>
                        <span class="lsa-category-count"><?= (int)($cat['article_count'] ?? 0) ?></span>
                    </a>
                <?php endforeach; ?>
            <?php else: ?>
                <p class="lsa-no-categories">No categories yet.</p>
            <?php endif; ?>
        </div>
    </div>

    <div class="lsa-sidebar-widget lsa-sidebar-about">
        <h4 class="lsa-sidebar-title">About Our Blog</h4>
        <p class="lsa-sidebar-about-text" data-ts="sidebar.about">
            <?= esc(theme_get('sidebar.about', 'Dive into language learning tips, cultural insights, and success stories from our immersive academy. Discover how to master English, Spanish, French, or Mandarin through conversation.')) ?>
        </p>
    </div>

    <div class="lsa-sidebar-widget lsa-sidebar-tags">
        <h4 class="lsa-sidebar-title">Popular Tags</h4>
        <div class="lsa-tags-cloud">
            <a href="/articles?tag=conversation" class="lsa-tag">Conversation</a>
            <a href="/articles?tag=grammar" class="lsa-tag">Grammar</a>
            <a href="/articles?tag=culture" class="lsa-tag">Culture</a>
            <a href="/articles?tag=pronunciation" class="lsa-tag">Pronunciation</a>
            <a href="/articles?tag=immersion" class="lsa-tag">Immersion</a>
            <a href="/articles?tag=resources" class="lsa-tag">Resources</a>
            <a href="/articles?tag=success-stories" class="lsa-tag">Success Stories</a>
            <a href="/articles?tag=online-learning" class="lsa-tag">Online Learning</a>
        </div>
    </div>

    <div class="lsa-sidebar-widget lsa-sidebar-social">
        <h4 class="lsa-sidebar-title">Follow Along</h4>
        <p class="lsa-sidebar-social-desc">Get daily language tips and updates.</p>
        <div class="lsa-social-links">
            <?php if (theme_get('footer.facebook')): ?>
                <a href="<?= esc(theme_get('footer.facebook')) ?>" target="_blank" class="lsa-social-icon" aria-label="Facebook">
                    <i class="fab fa-facebook-f"></i>
                </a>
            <?php endif; ?>
            <?php if (theme_get('footer.instagram')): ?>
                <a href="<?= esc(theme_get('footer.instagram')) ?>" target="_blank" class="lsa-social-icon" aria-label="Instagram">
                    <i class="fab fa-instagram"></i>
                </a>
            <?php endif; ?>
            <?php if (theme_get('footer.twitter')): ?>
                <a href="<?= esc(theme_get('footer.twitter')) ?>" target="_blank" class="lsa-social-icon" aria-label="Twitter">
                    <i class="fab fa-twitter"></i>
                </a>
            <?php endif; ?>
            <?php if (theme_get('footer.linkedin')): ?>
                <a href="<?= esc(theme_get('footer.linkedin')) ?>" target="_blank" class="lsa-social-icon" aria-label="LinkedIn">
