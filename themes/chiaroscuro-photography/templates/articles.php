...
<div class="atpl-wrap">
    ...
    <!-- Search Widget -->
    <div class="cp-sidebar-widget cp-sidebar-search">
        <h4 class="cp-sidebar-title">Search Articles</h4>
        <form class="cp-search-form" role="search">
            <div class="cp-search-input-wrap">
                <input type="text" placeholder="Type keywords..." class="cp-search-input">
                <button type="submit" class="cp-search-btn" aria-label="Search">
    ...
    <div class="cp-sidebar-widget cp-sidebar-categories">
        <h4 class="cp-sidebar-title">Categories</h4>
        <div class="cp-categories-list">
            <?php foreach ($categories as $cat): ?>
            <a href="/articles?category=<?= esc($cat['slug']) ?>" class="cp-category-link">
                <span class="cp-category-name"><?= esc($cat['name']) ?></span>
                <span class="cp-category-count"><?= (int)($cat['article_count'] ?? 0) ?></span>
            </a>
            <?php endforeach; ?>
        </div>
    </div>
    ...
    <!-- About This Blog -->
    <div class="cp-sidebar-widget cp-sidebar-about">
        <h4 class="cp-sidebar-title">About This Blog</h4>
        <p class="cp-sidebar-text" data-ts="sidebar.about">
            <?= esc(theme_get('sidebar.about', 'Insights, tutorials, and behind-the-scenes stories from an award-winning photography studio. Explore the art and craft of visual storytelling.')) ?>
        </p>
    </div>

    <!-- Newsletter Signup -->
    <div class="cp-sidebar-widget cp-sidebar-newsletter">
        <h4 class="cp-sidebar-title">Stay Inspired</h4>
        <p class="cp-sidebar-text">Get photography tips and exclusive content delivered to your inbox.</p>
        <form class="cp-newsletter-form">
            <input type="email" placeholder="Your email address" class="cp-newsletter-input" required>
            <button type="submit" class="cp-btn cp-btn-primary cp-btn-full">Subscribe</button>
        </form>
        <p class="cp-newsletter-note">No spam. Unsubscribe anytime.</p>
    </div>

    <!-- Social Follow -->
    <div class="cp-sidebar-widget cp-sidebar-social">
        <h4 class="cp-sidebar-title">Follow Us</h4>
        <div class="cp-social-links">
            <?php if (theme_get('footer.facebook')): ?>
            <a href="<?= esc(theme_get('footer.facebook')) ?>" target="_blank" rel="noopener" class="cp-social-link" aria-label="Facebook">
                <i class="fab fa-facebook-f"></i>
            </a>
            <?php endif; ?>
            <?php if (theme_get('footer.instagram')): ?>
            <a href="<?= esc(theme_get('footer.instagram')) ?>" target="_blank" rel="noopener" class="cp-social-link" aria-label="Instagram">
                <i class="fab fa-instagram"></i>
            </a>
            <?php endif; ?>
            <?php if (theme_get('footer.twitter')): ?>
            <a href="<?= esc(theme_get('footer.twitter')) ?>" target="_blank" rel="noopener" class="cp-social-link" aria-label="Twitter">
                <i class="fab fa-twitter"></i>
            </a>
            <?php endif; ?>
            <?php if (theme_get('footer.youtube')): ?>
            <a href="<?= esc(theme_get('footer.youtube')) ?>" target="_blank" rel="noopener" class="cp-social-link" aria-label="YouTube">
    ...
    <div class="cp-sidebar-widget cp-sidebar-recent">
        <h4 class="cp-sidebar-title">Latest Stories</h4>
        <div class="cp-recent-list">
            <div class="cp-recent-item">
                <div class="cp-recent-thumb"></div>
                <div class="cp-recent-content">
                    <h5 class="cp-recent-title">Mastering Natural Light Portraits</h5>
                    <span class="cp-recent-date">May 12, 2025</span>
                </div>
            </div>
            <div class="cp-recent-item">
                <div class="cp-recent-thumb"></div>
                <div class="cp-recent-content">
                    <h5 class="cp-recent-title">Behind the Lens: A Wedding Day</h5>
                    <span class="cp-recent-date">April 28, 2025</span>
                </div>
            </div>
            <div class="cp-recent-item">
                <div class="cp-recent-thumb"></div>
                <div class="cp-recent-content">
                    <h5 class="cp-recent-title">Essential Gear for Travel Photography</h5>
                    <span class="cp-recent-date">April 15, 2025</span>
                </div>
            </div>
        </div>
    </div>
