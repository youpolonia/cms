...
<div class="atpl-wrap">
    ...
    <aside class="sn-nexus-sidebar">
    <div class="sn-sidebar-card">
        <h4 class="sn-sidebar-title">Search Posts</h4>
        <form class="sn-sidebar-search" action="/articles" method="get">
            <div class="sn-search-input-wrap">
                <input type="text" name="q" placeholder="Type keywords..." class="sn-search-input">
    ...
    <div class="sn-sidebar-card">
        <h4 class="sn-sidebar-title">Categories</h4>
        <div class="sn-sidebar-categories">
            <?php if (!empty($categories)): ?>
                <?php foreach ($categories as $cat): ?>
                    <a href="/articles?category=<?= esc($cat['slug']) ?>" class="sn-cat-link">
                        <span class="sn-cat-name"><?= esc($cat['name']) ?></span>
                        <span class="sn-cat-count"><?= (int)($cat['article_count'] ?? 0) ?></span>
                    </a>
                <?php endforeach; ?>
            <?php else: ?>
                <p class="sn-sidebar-muted">No categories yet.</p>
            <?php endif; ?>
        </div>
    </div>

    <div class="sn-sidebar-card">
        <h4 class="sn-sidebar-title">About Our Blog</h4>
        <p class="sn-sidebar-about" data-ts="sidebar.about">
            <?= esc(theme_get('sidebar.about', 'Insights and updates from the team building the future of remote collaboration. Explore tutorials, product news, and remote work culture.')) ?>
        </p>
    </div>

    <div class="sn-sidebar-card">
        <h4 class="sn-sidebar-title">Stay Connected</h4>
        <div class="sn-sidebar-social">
            <?php if (theme_get('footer.facebook')): ?>
                <a href="<?= esc(theme_get('footer.facebook')) ?>" target="_blank" class="sn-social-icon" aria-label="Facebook">
                    <i class="fab fa-facebook-f"></i>
                </a>
            <?php endif; ?>
            <?php if (theme_get('footer.instagram')): ?>
                <a href="<?= esc(theme_get('footer.instagram')) ?>" target="_blank" class="sn-social-icon" aria-label="Instagram">
                    <i class="fab fa-instagram"></i>
                </a>
            <?php endif; ?>
            <?php if (theme_get('footer.linkedin')): ?>
                <a href="<?= esc(theme_get('footer.linkedin')) ?>" target="_blank" class="sn-social-icon" aria-label="LinkedIn">
                    <i class="fab fa-linkedin-in"></i>
                </a>
            <?php endif; ?>
            <?php if (theme_get('footer.twitter')): ?>
                <a href="<?= esc(theme_get('footer.twitter')) ?>" target="_blank" class="sn-social-icon" aria-label="Twitter">
    ...
    <div class="sn-sidebar-card">
        <h4 class="sn-sidebar-title">Popular Tags</h4>
        <div class="sn-tags-cloud">
            <a href="/articles?tag=remote-work" class="sn-tag">Remote Work</a>
            <a href="/articles?tag=productivity" class="sn-tag">Productivity</a>
            <a href="/articles?tag=integrations" class="sn-tag">Integrations</a>
            <a href="/articles?tag=video-calls" class="sn-tag">Video Calls</a>
            <a href="/articles?tag=channels" class="sn-tag">Channels</a>
            <a href="/articles?tag=devtools" class="sn-tag">DevTools</a>
        </div>
    </div>
</aside>
</div>