...
<div class="atpl-wrap">
    ...
    <?php
$sidebarAbout = theme_get('sidebar.about', 'Empowering freelancers with intelligent financial tools for the modern economy.');
$sidebarQuote = theme_get('sidebar.quote', 'Smart banking starts with understanding your money flow.');
?>
<aside class="vbf-sidebar">
    <?php if (!empty($categories)): ?>
    <div class="vbf-sidebar-module vbf-sidebar-categories">
        <h3 class="vbf-sidebar-heading">Categories</h3>
        <div class="vbf-category-grid">
            <?php foreach ($categories as $cat): ?>
            <a href="/articles?category=<?= esc($cat['slug']) ?>" class="vbf-category-chip">
                <span class="vbf-category-name"><?= esc($cat['name']) ?></span>
                <span class="vbf-category-count"><?= (int)($cat['article_count'] ?? 0) ?></span>
            </a>
            <?php endforeach; ?>
        </div>
    </div>
    ...
    <div class="vbf-sidebar-module vbf-sidebar-about">
        <div class="vbf-about-icon">
            <i class="fas fa-lightbulb"></i>
        </div>
        <h3 class="vbf-sidebar-heading">About Velocity Insights</h3>
        <p class="vbf-sidebar-text" data-ts="sidebar.about"><?= esc($sidebarAbout) ?></p>
    </div>
    
    <div class="vbf-sidebar-module vbf-sidebar-search">
        <h3 class="vbf-sidebar-heading">Search Articles</h3>
        <form class="vbf-search-form" role="search">
            <div class="vbf-search-input-wrap">
                <i class="fas fa-search"></i>
                <input type="text" placeholder="Search insights..." class="vbf-search-input">
            </div>
            <button type="submit" class="vbf-search-submit">Search</button>
        </form>
    </div>
    
    <div class="vbf-sidebar-module vbf-sidebar-quote">
        <div class="vbf-quote-mark">
            <i class="fas fa-quote-left"></i>
        </div>
        <blockquote class="vbf-quote-text" data-ts="sidebar.quote">
            <?= esc($sidebarQuote) ?>
        </blockquote>
    </div>
    
    <div class="vbf-sidebar-module vbf-sidebar-newsletter">
        <div class="vbf-newsletter-icon">
            <i class="fas fa-envelope"></i>
        </div>
        <h3 class="vbf-sidebar-heading">Stay Updated</h3>
        <p class="vbf-sidebar-text">Get the latest fintech insights delivered to your inbox.</p>
        <form class="vbf-newsletter-form">
            <input type="email" placeholder="your@email.com" class="vbf-newsletter-input" required>
            <button type="submit" class="vbf-newsletter-btn">Subscribe</button>
        </form>
    </div>
    
    <div class="vbf-sidebar-module vbf-sidebar-social">
        <h3 class="vbf-sidebar-heading">Follow Us</h3>
        <div class="vbf-sidebar-social-links">
            <?php if (theme_get('footer.facebook')): ?>
            <a href="<?= esc(theme_get('footer.facebook')) ?>" target="_blank" rel="noopener" class="vbf-sidebar-social-link" aria-label="Facebook">
                <i class="fab fa-facebook-f"></i>
            </a>
            <?php endif; ?>
            <?php if (theme_get('footer.instagram')): ?>
            <a href="<?= esc(theme_get('footer.instagram')) ?>" target="_blank" rel="noopener" class="vbf-sidebar-social-link" aria-label="Instagram">
                <i class="fab fa-instagram"></i>
            </a>
            <?php endif; ?>
            <?php if (theme_get('footer.linkedin')): ?>
            <a href="<?= esc(theme_get('footer.linkedin')) ?>" target="_blank" rel="noopener" class="vbf-sidebar-social-link" aria-label="LinkedIn">
                <i class="fab fa-linkedin-in"></i>
            </a>
            <?php endif; ?>
            <?php if (theme_get('footer.youtube')): ?>
            <a href="<?= esc(theme_get('footer.youtube')) ?>" target="_blank" rel="noopener" class="vbf-sidebar-social-link" aria-label="YouTube">
