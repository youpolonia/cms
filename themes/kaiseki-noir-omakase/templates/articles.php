...
<div class="atpl-wrap">
    ...
    <?php
$sidebarAbout = theme_get('sidebar.about', 'Discover the art of Japanese cuisine through our carefully curated articles on omakase traditions, seasonal ingredients, and culinary philosophy.');
$sidebarQuote = theme_get('sidebar.quote', 'The essence of omakase lies in trust—trust in the chef, trust in the season, trust in the moment.');
?>
<aside class="kno-articles-aside">
    <?php if (!empty($categories)): ?>
    <div class="kno-aside-module kno-aside-categories">
        <h4 class="kno-aside-heading">Culinary Categories</h4>
        <div class="kno-category-list">
            <?php foreach ($categories as $cat): ?>
            <a href="/articles?category=<?= esc($cat['slug']) ?>" class="kno-category-item">
                <span class="kno-category-name"><?= esc($cat['name']) ?></span>
                <span class="kno-category-badge"><?= (int)($cat['article_count'] ?? 0) ?></span>
            </a>
            <?php endforeach; ?>
        </div>
    </div>
    ...
    <div class="kno-aside-module kno-aside-quote">
        <div class="kno-quote-mark"><i class="fas fa-quote-left"></i></div>
        <blockquote class="kno-quote-text" data-ts="sidebar.quote"><?= esc($sidebarQuote) ?></blockquote>
    </div>

    <div class="kno-aside-module kno-aside-about">
        <h4 class="kno-aside-heading">About Our Journal</h4>
        <p class="kno-aside-text" data-ts="sidebar.about"><?= esc($sidebarAbout) ?></p>
    </div>

    <div class="kno-aside-module kno-aside-social">
        <h4 class="kno-aside-heading">Follow Our Journey</h4>
        <div class="kno-aside-social-grid">
            <?php if (theme_get('footer.instagram')): ?>
            <a href="<?= esc(theme_get('footer.instagram')) ?>" class="kno-social-tile" target="_blank" rel="noopener" aria-label="Instagram">
                <i class="fab fa-instagram"></i>
            </a>
            <?php endif; ?>
            <?php if (theme_get('footer.facebook')): ?>
            <a href="<?= esc(theme_get('footer.facebook')) ?>" class="kno-social-tile" target="_blank" rel="noopener" aria-label="Facebook">
                <i class="fab fa-facebook-f"></i>
            </a>
            <?php endif; ?>
            <?php if (theme_get('footer.twitter')): ?>
            <a href="<?= esc(theme_get('footer.twitter')) ?>" class="kno-social-tile" target="_blank" rel="noopener" aria-label="Twitter">
                <i class="fab fa-twitter"></i>
            </a>
            <?php endif; ?>
            <?php if (theme_get('footer.youtube')): ?>
            <a href="<?= esc(theme_get('footer.youtube')) ?>" class="kno-social-tile" target="_blank" rel="noopener" aria-label="YouTube">
    ...
    <div class="kno-aside-module kno-aside-newsletter">
        <h4 class="kno-aside-heading">Subscribe to Our Table</h4>
        <p class="kno-aside-text">Receive exclusive updates on seasonal menus, omakase experiences, and culinary insights.</p>
        <form class="kno-newsletter-form">
            <input type="email" placeholder="Your email address" class="kno-newsletter-input" required>
            <button type="submit" class="kno-newsletter-btn">
                <span>Subscribe</span>
