...
<div class="atpl-wrap">
    ...
    <div class="ss-sidebar-module ss-categories-module">
        <h4 class="ss-sidebar-heading">
            <i class="fas fa-folder-open"></i>
            <span>Security Topics</span>
        </h4>
        <div class="ss-category-list">
            <?php foreach ($categories as $cat): ?>
            <a href="/articles?category=<?= esc($cat['slug']) ?>" class="ss-category-link">
                <span class="ss-cat-name"><?= esc($cat['name']) ?></span>
                <span class="ss-cat-badge"><?= (int)($cat['article_count'] ?? 0) ?></span>
            </a>
            <?php endforeach; ?>
        </div>
    </div>
    ...
    <div class="ss-sidebar-module ss-about-module">
        <h4 class="ss-sidebar-heading">
            <i class="fas fa-shield-alt"></i>
            <span>About Our Research</span>
        </h4>
        <p class="ss-sidebar-text" data-ts="sidebar.about">
            <?= esc(theme_get('sidebar.about', 'Stay informed about the latest developments in zero-trust security, identity management, and compliance. Our research team analyzes emerging threats and shares actionable insights for enterprise security leaders.')) ?>
        </p>
    </div>
    
    <div class="ss-sidebar-module ss-newsletter-module">
        <div class="ss-newsletter-icon">
            <i class="fas fa-bell"></i>
        </div>
        <h4 class="ss-sidebar-heading">Security Alerts</h4>
        <p class="ss-sidebar-text">Get notified about critical security updates and compliance changes.</p>
        <form class="ss-newsletter-form">
            <input type="email" placeholder="Your work email" class="ss-newsletter-input" required>
            <button type="button" class="ss-newsletter-btn">
                <span>Subscribe</span>
                <i class="fas fa-arrow-right"></i>
            </button>
        </form>
        <p class="ss-newsletter-note">
            <i class="fas fa-lock"></i>
            <span>Your data is encrypted and never shared</span>
        </p>
    </div>
    
    <div class="ss-sidebar-module ss-quote-module">
        <div class="ss-quote-mark">
            <i class="fas fa-quote-left"></i>
        </div>
        <blockquote class="ss-security-tip" data-ts="sidebar.quote">
            <?= esc(theme_get('sidebar.quote', 'Zero-trust is not a product—it\'s a comprehensive security strategy that assumes no user or device is trusted by default, regardless of location.')) ?>
        </blockquote>
    </div>
</aside>
</div>