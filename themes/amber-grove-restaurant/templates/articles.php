...
<div class="atpl-wrap">
    ...
    <aside class="agr-articles-sidebar">
    <div class="agr-sidebar-search">
        <form class="agr-search-form" action="/articles" method="GET">
            <input type="text" name="q" placeholder="Search articles..." class="agr-search-input">
            <button type="submit" class="agr-search-btn" aria-label="Search">
                <i class="fas fa-search"></i>
            </button>
        </form>
    </div>

    <div class="agr-sidebar-block">
        <h4 class="agr-sidebar-title">Categories</h4>
        <div class="agr-sidebar-cats">
            <?php if (!empty($categories)): ?>
                <?php foreach ($categories as $cat): ?>
                    <a href="/articles?category=<?= esc($cat['slug']) ?>" class="agr-sidebar-cat-link">
                        <span class="agr-cat-name"><?= esc($cat['name']) ?></span>
                        <span class="agr-cat-count"><?= (int)($cat['article_count'] ?? 0) ?></span>
                    </a>
                <?php endforeach; ?>
            <?php else: ?>
                <p class="agr-sidebar-empty">No categories yet.</p>
            <?php endif; ?>
        </div>
    </div>

    <div class="agr-sidebar-block">
        <h4 class="agr-sidebar-title">From the Cellar</h4>
        <p class="agr-sidebar-desc" data-ts="sidebar.about">
            <?= esc(theme_get('sidebar.about', 'Insights on wine pairings, culinary techniques, and the stories behind our signature dishes.')) ?>
        </p>
    </div>

    <div class="agr-sidebar-block">
        <h4 class="agr-sidebar-title">Featured Tags</h4>
        <div class="agr-sidebar-tags">
            <a href="/articles" class="agr-tag">Wine Pairing</a>
            <a href="/articles" class="agr-tag">Seasonal Menu</a>
            <a href="/articles" class="agr-tag">Chef's Table</a>
            <a href="/articles" class="agr-tag">Farm to Table</a>
            <a href="/articles" class="agr-tag">Cocktail Craft</a>
            <a href="/articles" class="agr-tag">Dining Experience</a>
        </div>
    </div>

    <div class="agr-sidebar-block agr-sidebar-cta">
        <h4 class="agr-sidebar-title">Stay Curious</h4>
        <p>Receive our monthly digest of culinary stories and exclusive offers.</p>
        <form class="agr-sidebar-newsletter">
            <input type="email" placeholder="Your email address" class="agr-newsletter-input">
            <button type="button" class="agr-btn agr-btn--small">Subscribe</button>
        </form>
    </div>
</aside>
</div>