...
<div class="atpl-wrap">
    ...
    <aside class="tf-articles-sidebar">
    <div class="tf-sidebar-search">
        <form action="/articles" method="GET" class="tf-search-form">
            <input type="text" name="q" placeholder="Search articles..." class="tf-search-input">
            <button type="submit" class="tf-search-btn" aria-label="Search">
    ...
    <div class="tf-sidebar-block">
        <h4 class="tf-sidebar-title">Categories</h4>
        <div class="tf-categories-list">
            <?php foreach ($categories as $cat): ?>
            <a href="/articles?category=<?= esc($cat['slug']) ?>" class="tf-category-link">
                <span class="tf-category-name"><?= esc($cat['name']) ?></span>
                <span class="tf-category-count"><?= (int)($cat['article_count'] ?? 0) ?></span>
            </a>
            <?php endforeach; ?>
        </div>
    </div>
    ...
    <div class="tf-sidebar-block">
        <h4 class="tf-sidebar-title" data-ts="sidebar.about_title">Landscaping Insights</h4>
        <p class="tf-sidebar-about" data-ts="sidebar.about">
            <?= esc(theme_get('sidebar.about', 'Expert advice on outdoor living, hardscape design, and sustainable landscaping from our team of professionals.')) ?>
        </p>
    </div>

    <div class="tf-sidebar-block">
        <h4 class="tf-sidebar-title">Popular Tags</h4>
        <div class="tf-tags-cloud">
            <a href="/articles?tag=patios" class="tf-tag">Patios</a>
            <a href="/articles?tag=driveways" class="tf-tag">Driveways</a>
            <a href="/articles?tag=retaining-walls" class="tf-tag">Retaining Walls</a>
            <a href="/articles?tag=fencing" class="tf-tag">Fencing</a>
            <a href="/articles?tag=artificial-grass" class="tf-tag">Artificial Grass</a>
            <a href="/articles?tag=landscape-design" class="tf-tag">Design</a>
            <a href="/articles?tag=maintenance" class="tf-tag">Maintenance</a>
            <a href="/articles?tag=sustainability" class="tf-tag">Sustainability</a>
        </div>
    </div>

    <div class="tf-sidebar-block tf-sidebar-social">
        <h4 class="tf-sidebar-title">Follow Us</h4>
        <div class="tf-sidebar-social-links">
            <?php if (theme_get('footer.facebook')): ?>
                <a href="<?= esc(theme_get('footer.facebook')) ?>" target="_blank" class="tf-sidebar-social-link" aria-label="Facebook">
                    <i class="fab fa-facebook-f"></i>
                </a>
            <?php endif; ?>
            <?php if (theme_get('footer.instagram')): ?>
                <a href="<?= esc(theme_get('footer.instagram')) ?>" target="_blank" class="tf-sidebar-social-link" aria-label="Instagram">
                    <i class="fab fa-instagram"></i>
                </a>
            <?php endif; ?>
            <?php if (theme_get('footer.youtube')): ?>
                <a href="<?= esc(theme_get('footer.youtube')) ?>" target="_blank" class="tf-sidebar-social-link" aria-label="YouTube">
