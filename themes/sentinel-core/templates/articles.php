...
<div class="atpl-wrap">
    ...
    <aside class="sc-articles-sidebar">
    <div class="sc-sidebar-search">
        <h4 class="sc-sidebar-title">Search Articles</h4>
        <form class="sc-search-form" role="search">
            <input type="search" class="sc-search-input" placeholder="Enter keywords..." aria-label="Search articles">
            <button type="submit" class="sc-search-btn" aria-label="Submit search"><i class="fas fa-search"></i></button>
        </form>
    </div>

    <div class="sc-sidebar-block">
        <h4 class="sc-sidebar-title">Categories</h4>
        <div class="sc-categories-list">
            <?php if (!empty($categories)): ?>
                <?php foreach ($categories as $cat): ?>
                    <a href="/articles?category=<?= esc($cat['slug']) ?>" class="sc-category-link">
                        <span class="sc-category-name"><?= esc($cat['name']) ?></span>
                        <span class="sc-category-count"><?= (int)($cat['article_count'] ?? 0) ?></span>
                    </a>
                <?php endforeach; ?>
            <?php else: ?>
                <p class="sc-sidebar-text">No categories yet.</p>
            <?php endif; ?>
        </div>
    </div>

    <div class="sc-sidebar-block">
        <h4 class="sc-sidebar-title">About Our Insights</h4>
        <p class="sc-sidebar-text" data-ts="sidebar.about">
            <?= esc(theme_get('sidebar.about', 'Expert analysis on AI threat detection, zero-day prevention, and enterprise security strategies from Sentinel Core.')) ?>
        </p>
    </div>

    <div class="sc-sidebar-block">
        <h4 class="sc-sidebar-title">Stay Protected</h4>
        <p class="sc-sidebar-text">Get the latest threat intelligence delivered to your inbox.</p>
        <form class="sc-newsletter-form">
            <input type="email" class="sc-newsletter-input" placeholder="Your work email" required>
            <button type="button" class="sc-btn sc-btn-secondary sc-btn-small">Subscribe</button>
        </form>
    </div>

    <div class="sc-sidebar-block">
        <h4 class="sc-sidebar-title">Follow Us</h4>
        <div class="sc-sidebar-social">
            <?php if (theme_get('footer.facebook')): ?>
                <a href="<?= esc(theme_get('footer.facebook')) ?>" class="sc-social-icon" target="_blank" rel="noopener" aria-label="Facebook"><i class="fab fa-facebook-f"></i></a>
            <?php endif; ?>
            <?php if (theme_get('footer.twitter')): ?>
                <a href="<?= esc(theme_get('footer.twitter')) ?>" class="sc-social-icon" target="_blank" rel="noopener" aria-label="Twitter"><i class="fab fa-twitter"></i></a>
            <?php endif; ?>
            <?php if (theme_get('footer.linkedin')): ?>
                <a href="<?= esc(theme_get('footer.linkedin')) ?>" class="sc-social-icon" target="_blank" rel="noopener" aria-label="LinkedIn"><i class="fab fa-linkedin-in"></i></a>
            <?php endif; ?>
            <?php if (theme_get('footer.github')): ?>
                <a href="<?= esc(theme_get('footer.github')) ?>" class="sc-social-icon" target="_blank" rel="noopener" aria-label="GitHub"><i class="fab fa-github"></i></a>
            <?php endif; ?>
        </div>
    </div>
</aside>
